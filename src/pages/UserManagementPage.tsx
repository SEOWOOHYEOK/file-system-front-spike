...
/** 부서명에서 상위 본부 추출 (규칙 기반) */
function inferDivision(departmentName: string): string {
  // 본부명을 포함하는 경우
  if (departmentName.includes('경영지원') || departmentName.includes('사업개발')) {
    return '경영지원본부';
  }
  if (
    departmentName.includes('PM') ||
    departmentName.includes('시스템') ||
    departmentName.includes('ES') ||
    departmentName.includes('IP') ||
    departmentName.includes('EP') ||
    departmentName.includes('EC') ||
    departmentName.includes('RF') ||
    departmentName.includes('MEDIA') ||
    departmentName.includes('연구') ||
    departmentName.includes('개발') ||
    departmentName.endsWith('파트')
  ) {
    return '연구개발본부';
  }
  return '기타';
...
function buildOrgTree(users: UserWithEmployee[]): OrgNode {
  // 부서별 사용자 수 집계
  const deptUserCount = new Map<string, Set<string>>();

  for (const user of users) {
    if (!user.employee?.departmentPositions) continue;
    for (const dp of user.employee.departmentPositions) {
      if (!deptUserCount.has(dp.departmentName)) {
        deptUserCount.set(dp.departmentName, new Set());
      }
      deptUserCount.get(dp.departmentName)!.add(user.id);
    }
  }
  ...
  for (const [deptName, userIds] of deptUserCount) {
    const division = inferDivision(deptName);
    if (!divisionMap.has(division)) {
      divisionMap.set(division, new Map());
    }
    divisionMap.get(division)!.set(deptName, userIds.size);
  }

  // 트리 노드 생성
  const divisionNodes: OrgNode[] = [];

  for (const [divName, departments] of divisionMap) {
    const deptNodes: OrgNode[] = [];
    let divTotal = 0;

    for (const [deptName, count] of departments) {
      deptNodes.push({
        id: `dept-${deptName}`,
        name: deptName,
        children: [],
    ...
    // 부서명 정렬
    deptNodes.sort((a, b) => a.name.localeCompare(b.name, 'ko'));
  ...
  // 본부 정렬 (경영지원 → 연구개발 → 기타)
  const order = ['경영지원본부', '연구개발본부', '기타'];
  divisionNodes.sort((a, b) => {
...
export function UserManagementPage() {
  ...
  }, [auth.isAuthenticated, fetchUsers, fetchRoles]);

  // 조직 트리 빌드
  useEffect(() => {
    if (users.length > 0) {
      setOrgTree(buildOrgTree(users));
    }
  }, [users]);
  ...
  const toggleNode = useCallback((nodeId: string) => {
    setOrgTree((prev) => {
      if (!prev) return null;
      const toggle = (node: OrgNode): OrgNode => {
        if (node.id === nodeId) {
  ...
  // ── 부서 선택 ──
  const handleDeptSelect = useCallback((deptName: string | null) => {
    setSelectedDepartment((prev) => (prev === deptName ? null : deptName));
  }, []);
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (selectedDepartment) {
      result = result.filter((user) =>
        user.employee?.departmentPositions.some(
          (dp) => dp.departmentName === selectedDepartment
        )
      );
    }
    ...
    return result;
  }, [users, selectedDepartment, roleFilter, searchQuery, sortField, sortDir]);
  ...
  const getPrimaryDepartment = (
    ...
    // 선택된 부서가 있으면 해당 부서 정보 우선
    if (selectedDepartment) {
      const match = dps.find((dp) => dp.departmentName === selectedDepartment);
  ...
  const renderOrgNode = (node: OrgNode, depth: number = 0) => {
    ...
    const isSelected =
      node.level === 'department' && selectedDepartment === node.name;
    ...
    return (
      <div key={node.id}>
        ...
        >
          ...
          {/* 아이콘 */}
          {node.level === 'company' ? (
            <svg className="w-4 h-4 mr-1.5 text-gray-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-4 border-b border-gray-100">
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                // 부서 검색 시 트리 필터링 (간단 구현: 부서명 포함 여부)
                ...
                // 필터된 트리
                const fullTree = buildOrgTree(users);
                ...
                if (filtered) setOrgTree(filtered);
              }}
            />
          </div>
        </div>
