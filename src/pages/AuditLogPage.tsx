...
const ACTION_LABELS: Record<string, string> = {
  FILE_VIEW: '파일 조회', FILE_DOWNLOAD: '파일 다운로드', FILE_UPLOAD: '파일 업로드',
  FILE_RENAME: '파일 이름 변경', FILE_MOVE: '파일 이동', FILE_DELETE: '파일 삭제',
  FILE_RESTORE: '파일 복원', FILE_PURGE: '파일 영구 삭제',
  FOLDER_CREATE: '폴더 생성', FOLDER_VIEW: '폴더 조회', FOLDER_RENAME: '폴더 이름 변경',
  FOLDER_MOVE: '폴더 이동', FOLDER_DELETE: '폴더 삭제',
  SHARE_CREATE: '공유 생성', SHARE_REVOKE: '공유 해제', SHARE_ACCESS: '공유 접근',
  SHARE_DOWNLOAD: '공유 다운로드', SHARE_BLOCK: '공유 차단', SHARE_UNBLOCK: '공유 차단 해제',
  SHARE_BULK_BLOCK: '공유 일괄 차단', SHARE_BULK_UNBLOCK: '공유 일괄 차단 해제',
  SHARE_REQUEST_CREATE: '공유 요청', SHARE_REQUEST_APPROVE: '공유 요청 승인',
  SHARE_REQUEST_REJECT: '공유 요청 거부', SHARE_REQUEST_CANCEL: '공유 요청 취소',
  SHARE_REQUEST_BULK_APPROVE: '공유 일괄 승인', SHARE_REQUEST_BULK_REJECT: '공유 일괄 거부',
  PERMISSION_GRANT: '권한 부여', PERMISSION_REVOKE: '권한 회수', PERMISSION_CHANGE: '권한 변경',
  TRASH_EMPTY: '휴지통 비우기', TRASH_VIEW: '휴지통 조회',
  FAVORITE_ADD: '즐겨찾기 등록', FAVORITE_REMOVE: '즐겨찾기 해제', FAVORITE_VIEW: '즐겨찾기 조회',
  ACTIVITY_VIEW: '최근 활동 조회',
  EXTERNAL_USER_CREATE: '외부 사용자 생성', EXTERNAL_USER_UPDATE: '외부 사용자 수정',
  EXTERNAL_USER_DEACTIVATE: '외부 사용자 비활성화', EXTERNAL_USER_ACTIVATE: '외부 사용자 활성화',
  EXTERNAL_USER_PASSWORD_RESET: '외부 사용자 비밀번호 초기화',
  PASSWORD_CHANGE: '비밀번호 변경',
  USER_ROLE_ASSIGN: '역할 부여', USER_ROLE_REMOVE: '역할 제거',
  USER_SYNC: '사용자 동기화', TOKEN_GENERATE: '토큰 생성', TOKEN_REFRESH: '토큰 갱신',
  ORG_MIGRATION: '조직 마이그레이션',
  FILE_ACTION_REQUEST_MOVE_CREATE: '파일 이동 요청', FILE_ACTION_REQUEST_DELETE_CREATE: '파일 삭제 요청',
  FILE_ACTION_REQUEST_CANCEL: '파일 요청 취소', FILE_ACTION_REQUEST_APPROVE: '파일 요청 승인',
  ...
  EXTERNAL_SHARE_DETAIL: '외부 공유 상세', EXTERNAL_SHARE_ACCESS: '외부 공유 접근',
  EXTERNAL_SHARE_DOWNLOAD: '외부 공유 다운로드',
  LOGIN_SUCCESS: '로그인', LOGIN_FAILURE: '로그인 실패', LOGOUT: '로그아웃',
  TOKEN_EXPIRED: '토큰 만료', PERMISSION_DENIED: '권한 거부',
  EXPIRED_LINK_ACCESS: '만료 링크 접근', BLOCKED_SHARE_ACCESS: '차단 공유 접근',
  ACCESS_PATTERN_DEVIATION: '접근 패턴 이탈', NEW_DEVICE_ACCESS: '신규 기기 접근',
};

const FILE_CHANGE_LABELS: Record<string, string> = {
  CREATED: '생성', CONTENT_REPLACED: '내용 교체', RENAMED: '이름 변경',
  MOVED: '이동', METADATA_CHANGED: '메타데이터 변경',
  TRASHED: '휴지통 이동', RESTORED: '복원', DELETED: '영구 삭제',
};
...
type ViewTab = 'audit' | 'file-history' | 'timeline';
...
function getTimeRange(period: string): { from: string; to: string } {
  ...
  let fromDate: Date;
  switch (period) {
    case '1h': fromDate = new Date(now.getTime() - 60 * 60 * 1000); break;
    case '6h': fromDate = new Date(now.getTime() - 6 * 60 * 60 * 1000); break;
...
function getCategoryLabel(category: ActionCategory): string {
  const labels: Record<ActionCategory, string> = {
    file: '파일 조작', folder: '파일 조작', share: '공유/요청 처리',
    auth: '사용자 행위', admin: '권한 변경', user: '사용자 행위',
    security: '보안 이벤트', external: '공유/요청 처리',
  };
...
function FilterPanel({
  ...
  const counts = useMemo(() => {
    const byType: Record<EventTypeCategory, number> = {
      all: logs.length, user_activity: 0, permission: 0,
      file_operation: 0, share_request: 0, security: 0,
    };
    const byResult: Record<ResultFilter, number> = { all: logs.length, SUCCESS: 0, FAIL: 0 };
  ...
  return (
    <div className="w-44 shrink-0 border-r border-gray-200 bg-white overflow-y-auto">
      ...
      <div className="p-3 border-b border-gray-100">
        <h3 className="text-xs font-semibold text-gray-500 mb-2">이벤트 타입</h3>
        <ul className="space-y-0.5">
          {(Object.keys(EVENT_TYPE_LABELS) as EventTypeCategory[]).map((key) => (
            <li key={key}>
              <button
              ...
              >
                ...
                <span className={`text-xs ${eventFilter === key ? 'text-blue-600' : 'text-gray-400'}`}>
                  {counts.byType[key]}
                </span>
              </button>
            </li>
        ...
        <ul className="space-y-0.5">
          {(['all', 'SUCCESS', 'FAIL'] as ResultFilter[]).map((key) => (
            <li key={key}>
              ...
              >
                <span>{key === 'all' ? '전체' : key === 'SUCCESS' ? '성공' : '실패'}</span>
                <span className={`text-xs ${resultFilter === key ? 'text-blue-600' : 'text-gray-400'}`}>
                  {counts.byResult[key]}
                </span>
              </button>
            </li>
...
function TimelineChart({ logs }: { logs: AuditLog[] }) {
  const chartData = useMemo(() => {
    ...
    const buckets = new Map<string, { time: string; count: number; success: number; fail: number }>();
    for (const log of logs) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} barSize={6}>
          ...
          <Tooltip
            ...
            formatter={(value, name) => [String(value), name === 'success' ? '성공' : '실패']}
          />
          <Bar dataKey="success" fill="#10b981" stackId="a" radius={[1, 1, 0, 0]} />
          <Bar dataKey="fail" fill="#ef4444" stackId="a" radius={[1, 1, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
      ...
      <table className="w-full text-xs">
        <thead className="bg-gray-50 sticky top-0 z-10">
          <tr className="text-left text-gray-500 border-b border-gray-200">
            ...
            <th className="px-3 py-2 font-medium whitespace-nowrap">이벤트명</th>
            <th className="px-3 py-2 font-medium whitespace-nowrap">이벤트 타입</th>
            ...
            >
              ...
              <td className="px-3 py-2 whitespace-nowrap">
                <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-medium ${
                  EVENT_TYPE_COLORS[log.actionCategory] || 'bg-gray-100 text-gray-600'
                }`}>
...
function FileHistoryTable({
  ...
  if (items.length === 0) {
    return <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">파일 이력이 없습니다.</div>;
  }
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          {items.map((item) => (
            ...
            >
              <td className="px-3 py-2 whitespace-nowrap text-gray-600">{formatDate(item.createdAt)}</td>
              <td className="px-3 py-2 whitespace-nowrap font-mono text-gray-500 max-w-[100px] truncate">{item.fileId}</td>
              <td className="px-3 py-2 whitespace-nowrap text-gray-700">v{item.version}</td>
              <td className="px-3 py-2 whitespace-nowrap">
              ...
              <td className="px-3 py-2 whitespace-nowrap font-mono text-gray-500 max-w-[100px] truncate">{item.changedBy}</td>
              <td className="px-3 py-2 text-gray-600 max-w-[160px] truncate">{item.changeSummary || '-'}</td>
              <td className="px-3 py-2 text-gray-600 max-w-[240px] truncate">{item.description}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function TimelineTable({
  ...
  if (events.length === 0) {
    return <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">타임라인 이벤트가 없습니다.</div>;
  }
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          {events.map((event) => (
            ...
            >
              <td className="px-3 py-2 whitespace-nowrap text-gray-600">{formatDate(event.occurredAt)}</td>
              <td className="px-3 py-2 whitespace-nowrap">
                <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-medium ${
                  event.eventSource === 'AUDIT' ? 'bg-blue-100 text-blue-800' :
                  event.eventSource === 'FILE_CHANGE' ? 'bg-indigo-100 text-indigo-800' :
              ...
              <td className="px-3 py-2 whitespace-nowrap font-medium text-gray-800">
                {ACTION_LABELS[event.eventType] || FILE_CHANGE_LABELS[event.eventType] || event.eventType}
              </td>
              <td className="px-3 py-2 whitespace-nowrap">
                <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-medium ${
              ...
              <td className="px-3 py-2 whitespace-nowrap text-gray-700">{event.actorName || event.actorId}</td>
              <td className="px-3 py-2 whitespace-nowrap text-gray-700 max-w-[140px] truncate">{event.targetName || '-'}</td>
              <td className="px-3 py-2 whitespace-nowrap text-gray-500 font-mono">
                {event.httpMethod && event.apiEndpoint ? `${event.httpMethod} ${event.apiEndpoint}` : '-'}
                {event.responseStatusCode ? ` ${event.responseStatusCode}` : ''}
              </td>
              <td className="px-3 py-2 text-gray-600 max-w-[240px] truncate">{event.description}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      {auditLog && (
        <div className="p-3 space-y-3 text-xs">
          <Section title="기본 정보">
            ...
            <Field label="이벤트" value={ACTION_LABELS[auditLog.action] || auditLog.action} />
            <Field label="카테고리" value={getCategoryLabel(auditLog.actionCategory)} badge={EVENT_TYPE_COLORS[auditLog.actionCategory]} />
            <Field label="결과" value={auditLog.result === 'SUCCESS' ? '성공' : '실패'} badge={RESULT_COLORS[auditLog.result]} />
            {auditLog.failReason && <Field label="실패 사유" value={auditLog.failReason} />}
            <Field label="시간" value={formatDate(auditLog.createdAt)} />
            {auditLog.durationMs != null && <Field label="소요 시간" value={`${auditLog.durationMs}ms`} />}
          </Section>
          ...
          <Section title="대상">
            ...
            {auditLog.sensitivity && <Field label="기밀 등급" value={auditLog.sensitivity} />}
            {onViewTargetLogs && (
              <button
                onClick={() => onViewTargetLogs(auditLog.targetType, auditLog.targetId)}
                className="text-blue-600 hover:underline text-[10px] mt-1"
              >
                이 대상의 접근 이력 보기
              </button>
          ...
          <Section title="클라이언트">
            ...
            <Field label="User-Agent" value={auditLog.userAgent} className="break-all" />
          </Section>

          <Section title="API 컨텍스트">
            {auditLog.httpMethod && <Field label="메서드" value={auditLog.httpMethod} />}
            {auditLog.apiEndpoint && <Field label="엔드포인트" value={auditLog.apiEndpoint} mono />}
            {auditLog.responseStatusCode && <Field label="응답 코드" value={String(auditLog.responseStatusCode)} />}
          </Section>
          ...
          <Section title="추적">
            ...
            {auditLog.parentEventId && (
              <>
                <Field label="부모 이벤트" value={auditLog.parentEventId} mono />
                {onViewEventChain && (
                  <button
          ...
          {auditLog.severity && (
            <Section title="보안">
              <Field label="심각도" value={auditLog.severity} badge={
                auditLog.severity === 'CRITICAL' ? 'bg-red-100 text-red-800' :
                auditLog.severity === 'HIGH' ? 'bg-orange-100 text-orange-800' :
                auditLog.severity === 'WARN' ? 'bg-yellow-100 text-yellow-800' :
                'bg-gray-100 text-gray-600'
              } />
              {auditLog.errorCode && <Field label="에러 코드" value={auditLog.errorCode} />}
            </Section>
          )}
      ...
      {fileHistory && (
        <div className="p-3 space-y-3 text-xs">
          <Section title="기본 정보">
            ...
            <Field label="변경 유형" value={FILE_CHANGE_LABELS[fileHistory.changeType] || fileHistory.changeType} />
          ...
          {(fileHistory.checksumBefore || fileHistory.checksumAfter) && (
            <Section title="체크섬">
              {fileHistory.checksumBefore && <Field label="이전" value={fileHistory.checksumBefore} mono />}
              {fileHistory.checksumAfter && <Field label="이후" value={fileHistory.checksumAfter} mono />}
            </Section>
          )}

          <Section title="설명">
            <p className="text-gray-700">{fileHistory.description}</p>
            {fileHistory.changeSummary && <p className="text-gray-500 mt-1">{fileHistory.changeSummary}</p>}
          </Section>
        </div>
      )}
      ...
      {timelineEvent && (
        <div className="p-3 space-y-3 text-xs">
          <Section title="기본 정보">
            ...
            <Field label="이벤트" value={ACTION_LABELS[timelineEvent.eventType] || timelineEvent.eventType} />
            <Field label="결과" value={timelineEvent.result === 'SUCCESS' ? '성공' : '실패'} badge={RESULT_COLORS[timelineEvent.result]} />
            <Field label="시간" value={formatDate(timelineEvent.occurredAt)} />
            {timelineEvent.durationMs != null && <Field label="소요 시간" value={`${timelineEvent.durationMs}ms`} />}
            {timelineEvent.severity && <Field label="심각도" value={timelineEvent.severity} />}
          </Section>

          <Section title="행위자/대상">
            <Field label="행위자" value={timelineEvent.actorName || timelineEvent.actorId} />
            <Field label="대상" value={timelineEvent.targetName || timelineEvent.targetId || '-'} />
          </Section>

          {(timelineEvent.httpMethod || timelineEvent.apiEndpoint) && (
            <Section title="API">
              {timelineEvent.httpMethod && <Field label="메서드" value={timelineEvent.httpMethod} />}
              {timelineEvent.apiEndpoint && <Field label="엔드포인트" value={timelineEvent.apiEndpoint} mono />}
              {timelineEvent.responseStatusCode && <Field label="응답 코드" value={String(timelineEvent.responseStatusCode)} />}
            </Section>
          )}

          {timelineEvent.parentEventId && (
            <Section title="추적">
              {timelineEvent.requestId && <Field label="요청 ID" value={timelineEvent.requestId} mono />}
              {timelineEvent.traceId && <Field label="트레이스 ID" value={timelineEvent.traceId} mono />}
              <Field label="부모 이벤트" value={timelineEvent.parentEventId} mono />
              {onViewEventChain && (
                <button
            ...
            <Section title="인과관계 체인">
              <div className="space-y-2">
                {eventChain.map((ev, idx) => (
                  <div key={ev.id} className="relative pl-4 border-l-2 border-blue-200">
...
function Field({
  label, value, mono, badge, className,
}: {
  label: string;
  value: string;
  mono?: boolean;
  ...
  return (
    <div className={`flex items-start gap-2 ${className || ''}`}>
      ...
      {badge ? (
        <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${badge}`}>{value}</span>
      ) : (
        <span className={`text-gray-700 break-all ${mono ? 'font-mono text-[10px]' : ''}`}>{value}</span>
      )}
    </div>
  );
}
          ...
          return (
            <button
              ...
              className={`px-2 py-1 rounded border ${
                p === page ? 'bg-blue-500 text-white border-blue-500' : 'border-gray-300 hover:bg-gray-50'
              }`}
            >
              {p}
            </button>
...
function TimelineSummary({ summary }: { summary: UnifiedTimelineResponse['summary'] }) {
  return (
    <div className="flex items-center gap-4 px-4 py-2 bg-gray-50 border-b border-gray-200 text-xs">
      <span className="text-gray-500">총 <strong className="text-gray-800">{summary.total}</strong>건</span>
      <span className="text-gray-300">|</span>
      {Object.entries(summary.bySource).map(([source, count]) => (
        <span key={source} className="text-gray-500">
          <span className={`inline-block w-2 h-2 rounded-full mr-1 ${
            source === 'AUDIT' ? 'bg-blue-400' : source === 'FILE_CHANGE' ? 'bg-indigo-400' : 'bg-gray-400'
          }`} />
          {source}: <strong>{count}</strong>
        </span>
      ))}
      <span className="text-gray-300">|</span>
      <span className="text-green-600">성공: <strong>{summary.byResult.SUCCESS}</strong></span>
      <span className="text-red-600">실패: <strong>{summary.byResult.FAILURE}</strong></span>
    </div>
  );
}
...
export function AuditLogPage() {
  ...
  const [timelineResponse, setTimelineResponse] = useState<UnifiedTimelineResponse | null>(null);
  ...
  const [selectedFileHistory, setSelectedFileHistory] = useState<FileHistory | null>(null);
  const [selectedTimelineEvent, setSelectedTimelineEvent] = useState<ObservabilityEvent | null>(null);
  ...
  // ── 파일 이력 조회 ──
  const fetchFileHistories = useCallback(async (page = 1) => {
    setLoading(true);
    setError(null);
    try {
      const { from, to } = getTimeRange(period);
  ...
  useEffect(() => {
    ...
    if (viewTab === 'audit') fetchAuditLogs(1);
    else if (viewTab === 'file-history') fetchFileHistories(1);
    else if (viewTab === 'timeline') fetchTimeline(1);
  }, [auth.isAuthenticated, viewTab, period, fetchAuditLogs, fetchFileHistories, fetchTimeline]);
  ...
  const filteredAuditLogs = useMemo(() => {
    ...
    // 이벤트 타입 필터
    if (eventFilter !== 'all') {
      filtered = filtered.filter((log) => CATEGORY_MAP[log.actionCategory] === eventFilter);
    }
  ...
  const handleSelectAuditLog = useCallback(async (log: AuditLog) => {
    closeDetail();
    try {
  ...
  const handleSelectFileHistory = useCallback((item: FileHistory) => {
    closeDetail();
    setSelectedFileHistory(item);
  }, [closeDetail]);

  const handleSelectTimelineEvent = useCallback((event: ObservabilityEvent) => {
    closeDetail();
    setSelectedTimelineEvent(event);
  }, [closeDetail]);

  const handleViewEventChain = useCallback(async (eventId: string) => {
    try {
  ...
  const handleViewUserLogs = useCallback(async (userId: string) => {
    setLoading(true);
    try {
  ...
  const handleViewTargetLogs = useCallback(async (targetType: string, targetId: string) => {
    setLoading(true);
    try {
      const logs = await auditLogApi.getAuditLogsByTarget(targetType as any, targetId, 100);
      setAuditLogs(logs);
      setAuditTotal(logs.length);
      setAuditTotalPages(1);
      setAuditPage(1);
  ...
  const handleViewSessionLogs = useCallback(async (sessionId: string) => {
    setLoading(true);
    try {
  ...
  const handleViewRequestTimeline = useCallback(async (requestId: string) => {
    setLoading(true);
    try {
  ...
  const handleViewTraceTimeline = useCallback(async (traceId: string) => {
    setLoading(true);
    try {
  ...
  const handleRefresh = useCallback(() => {
    closeDetail();
    if (viewTab === 'audit') fetchAuditLogs(auditPage);
    else if (viewTab === 'file-history') fetchFileHistories(fileHistoryPage);
    else if (viewTab === 'timeline') fetchTimeline(timelinePage);
  }, [viewTab, auditPage, fileHistoryPage, timelinePage, fetchAuditLogs, fetchFileHistories, fetchTimeline, closeDetail]);
  ...
  const hasDetail = selectedAuditLog || selectedFileHistory || selectedTimelineEvent;
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">관리자 감사 로그 및 통합 타임라인 조회</p>
          </div>
          <button
        ...
        <div className="flex items-center gap-1 mt-3">
          {([
            ...
            <button
              key={tab.key}
              onClick={() => { setViewTab(tab.key); closeDetail(); }}
      ...
      {error && (
        <div className="mx-4 mt-2 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700 flex items-center justify-between">
          <span>{error}</span>
          <button onClick={handleRefresh} className="ml-4 px-3 py-1 text-xs bg-red-100 hover:bg-red-200 rounded">
            다시 시도
          </button>
        </div>
      )}
