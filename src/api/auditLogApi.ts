...
export const auditLogApi = {
  ...
  async getAuditLogs(
    ...
    const { data } = await apiClient.get<PaginatedResult<AuditLog>>(url, {
      baseURL: AUDIT_BASE,
    });
  ...
  async getAuditLogSummary(
    ...
    const url = `/v1/admin/audit-logs/summary${qs ? `?${qs}` : ""}`;
    const { data } = await apiClient.get<AuditLogSummary>(url, {
      baseURL: AUDIT_BASE,
    });
    log("GET", url, 200, data);
    return data;
  ...
  async getFileHistories(
    ...
    const { data } = await apiClient.get<PaginatedResult<FileHistory>>(url, {
      baseURL: AUDIT_BASE,
    });
    log("GET", url, 200, data);
    return data;
  ...
  async getFileHistoryByFile(
    ...
    const { data } = await apiClient.get<FileHistory[]>(url, {
      baseURL: AUDIT_BASE,
    });
    log("GET", url, 200, data);
    return data;
